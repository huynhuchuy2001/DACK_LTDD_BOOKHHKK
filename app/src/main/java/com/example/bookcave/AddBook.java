...
import com.example.bookcave.extras.PreviewBook;
        ...
        updateBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(TextUtils.isEmpty(up1)){
                    Toast.makeText(getApplicationContext(),"Vui lòng nhập giá bán",Toast.LENGTH_SHORT).show();
                    return;
                }
                if(TextUtils.isEmpty(urp1)){
                    Toast.makeText(getApplicationContext(),"Vui lòng nhập giá thuê",Toast.LENGTH_SHORT).show();
                    return;
                }
                if(TextUtils.isEmpty(udp1)){
                    Toast.makeText(getApplicationContext(),"Vui lòng nhập phí giao hàng",Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                updateBtn.setText("Thêm kho mới...");
                ...
                db.collection("SellingList").document(uniqueid).set(sellingBook).addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        updateBtn.setText("Thêm sách vào danh sách bán");
                        if(task.isSuccessful()) {
                            Toast.makeText(AddBook.this,"Đã thêm hàng thành công", Toast.LENGTH_LONG).show();
