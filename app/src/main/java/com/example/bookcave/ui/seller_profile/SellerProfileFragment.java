package com.example.bookcave.ui.seller_profile;
        ...
        updateButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(TextUtils.isEmpty(verifythis)){
                    ...
                    Toast.makeText(getActivity(),"Nhập email chính xác của bạn để lưu các thay đổi",Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                if(TextUtils.isEmpty(updateln)){
                    Toast.makeText(getActivity(),"Vui lòng nhập Họ của bạn",Toast.LENGTH_SHORT).show();
                    return;
                }
                if(TextUtils.isEmpty(updateadd)){
                    Toast.makeText(getActivity(),"Vui lòng nhập địa chỉ của bạn",Toast.LENGTH_SHORT).show();
                    return;
                }
                if(TextUtils.isEmpty(updatepc)){
                    Toast.makeText(getActivity(),"Vui lòng nhập mã pin",Toast.LENGTH_SHORT).show();
                    return;
                }
                if(TextUtils.isEmpty(updatephnoown)){
                    Toast.makeText(getActivity(),"Vui lòng nhập số điện thoại của bạn",Toast.LENGTH_SHORT).show();
                    return;
                }
                if(TextUtils.isEmpty(updatephnocom)){
                    Toast.makeText(getActivity(),"Vui lòng nhập số điện thoại công ty của bạn",Toast.LENGTH_SHORT).show();
                    return;
                }
                if(TextUtils.isEmpty(updatesn)){
                    Toast.makeText(getActivity(),"Vui lòng nhập Tên cửa hàng của bạn",Toast.LENGTH_SHORT).show();
                    return;
                }
                if(updatephnocom.length()!=10){
                    uphno_company.setError("Số điện thoại phải có 10 chữ số");
                    return;
                }
                if(updatephnoown.length()!=10){
                    uphno_owner.setError("Số phải có 10 chữ số");
                    return;
                }
                if(updatepc.length()!=6){
                    upincode.setError("Pincode phải có 6 chữ số");
                    return;
                }
                else{
                    if(verifythis.equals(semail)) {
                        ...
                        Toast.makeText(getActivity(),"Nhập email chính xác",Toast.LENGTH_LONG).show();
                    }
                }
            }
        });
        ...
        db.collection("Users").document(userid).set(user).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if(task.isSuccessful()) {
                    Toast.makeText(getActivity(),"Thông tin được cập nhật thành công", Toast.LENGTH_LONG).show();
