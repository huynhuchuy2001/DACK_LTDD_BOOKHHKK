        ...
        docRef.get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
            ...
            public void onSuccess(DocumentSnapshot documentSnapshot) {
                if (documentSnapshot.exists()) {
                    ...
                    stotalamount.setText(String.format("Tổng cộng : %s", String.valueOf(ta)));
                    spin.setText(pc);
                    saddress.setText(ad);
            ...
            public void onClick(View v) {
                FirebaseFirestore.getInstance().collection("Orders").document(orderid).update("updatedat",dateTime,"status",updateto).addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        ...
                        {
                            Toast.makeText(ViewOrderCustomer.this, "\uD83C\uDF89 Chúc mừng! Thưởng thức cuốn sách \uD83E\uDD73", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
            }
